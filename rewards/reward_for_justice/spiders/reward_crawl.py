...
class RewardCrawlSpider(scrapy.Spider):
    ...
    def parse_detail(self,response):
        ...
        try:
            ...
            About = None
        try:
            image = [i.find('img').get('src') for i in soup.find('div',attrs={'id':'gallery-1'}).find_all('a')]
        except:
            image = None
        
        try:
            dob_raw = soup.find('div',attrs={'data-id':'9a896ea'}).text.strip().split(';')[0]
            DOB_ISO = datetime.strptime(dob_raw,'%B %d, %Y').isoformat()
        except:
            DOB_ISO = None
